...
from app.core.deps import ensure_provider_assignment, get_admin_user, get_current_user, get_verified_provider, require_roles
...
from app.schemas.provider import (
    ...
    PatientProviderAssignmentResponse,
...
def serialize_assignment(assignment: PatientProviderAssignment) -> PatientProviderAssignmentResponse:
    return PatientProviderAssignmentResponse(
        id=assignment.id,
        ...
        request_message=assignment.request_message,
        initiated_by_user_id=assignment.initiated_by_user_id,
        responded_by_user_id=assignment.responded_by_user_id,
...
def get_assignment_or_404(assignment_id: int, db: Session) -> PatientProviderAssignment:
    assignment = (
        db.query(PatientProviderAssignment)
        .options(
            joinedload(PatientProviderAssignment.patient),
            joinedload(PatientProviderAssignment.provider),
        )
        .filter(PatientProviderAssignment.id == assignment_id)
        .first()
    )
    if not assignment:
...
def get_assignment_by_pair(patient_user_id: int, provider_user_id: int, db: Session) -> PatientProviderAssignment | None:
    return (
        db.query(PatientProviderAssignment)
        .options(
            joinedload(PatientProviderAssignment.patient),
        ...
        .filter(
            ...
            PatientProviderAssignment.provider_user_id == provider_user_id,
        )
        .first()
    )


def ensure_provider_is_approved(provider_user_id: int, db: Session) -> ProviderProfile:
    profile = get_provider_profile_or_404(provider_user_id, db)
    if profile.verification_status != "approved":
        raise HTTPException(status_code=400, detail="Provider must be approved before requesting connections")
    return profile


def upsert_connection_request(
    *,
    patient_user_id: int,
    ...
    assignment = get_assignment_by_pair(patient_user_id, provider_user_id, db)
    if assignment and assignment.status == "active":
        raise HTTPException(status_code=409, detail="Patient and provider are already connected")
    ...
    if assignment is None:
        ...
        assignment.initiated_by_user_id = initiated_by_user_id
        assignment.request_message = request_message
        assignment.responded_by_user_id = None
        assignment.responded_at = None

    db.commit()
    return get_assignment_or_404(assignment.id, db)
...
def create_patient_provider_assignment(
    ...
    if assignment is None:
        ...
        assignment.initiated_by_user_id = admin_user.id
        assignment.request_message = None
        assignment.responded_by_user_id = admin_user.id
        assignment.responded_at = datetime.now().astimezone()

    db.commit()
    return {"message": "Assignment saved"}


@router.get("/providers/directory", response_model=list[ProviderProfileResponse])
def list_provider_directory(current_user: User = Depends(require_roles("patient")), db: Session = Depends(get_db)):
    del current_user
    profiles = (
        db.query(ProviderProfile)
        .options(joinedload(ProviderProfile.user))
...
@router.get("/patients/directory", response_model=list[UserResponse])
def list_patient_directory(provider_user: User = Depends(get_verified_provider), db: Session = Depends(get_db)):
    del provider_user
    patients = db.query(User).filter(User.role == "patient").order_by(User.name.asc(), User.id.asc()).all()
    return [to_user_response(patient) for patient in patients]


@router.get("/connections/mine", response_model=list[PatientProviderAssignmentResponse])
def list_my_connections(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    query = db.query(PatientProviderAssignment).options(
        joinedload(PatientProviderAssignment.patient),
        joinedload(PatientProviderAssignment.provider),
    )
    if current_user.role == "provider":
        query = query.filter(PatientProviderAssignment.provider_user_id == current_user.id)
    else:
        query = query.filter(PatientProviderAssignment.patient_user_id == current_user.id)
    assignments = query.order_by(PatientProviderAssignment.created_at.desc(), PatientProviderAssignment.id.desc()).all()
    return [serialize_assignment(assignment) for assignment in assignments]


@router.post("/connections/request/patient", response_model=PatientProviderAssignmentResponse, status_code=status.HTTP_201_CREATED)
def request_provider_connection(
    payload: ConnectionRequestCreate,
    current_user: User = Depends(require_roles("patient")),
    db: Session = Depends(get_db),
):
    if payload.provider_user_id is None:
        raise HTTPException(status_code=400, detail="Provider is required")
    get_patient_user_or_404(current_user.id, db)
    ensure_provider_is_approved(payload.provider_user_id, db)
    assignment = upsert_connection_request(
...
@router.post("/connections/request/provider", response_model=PatientProviderAssignmentResponse, status_code=status.HTTP_201_CREATED)
def request_patient_connection(
    payload: ConnectionRequestCreate,
    current_user: User = Depends(get_verified_provider),
    db: Session = Depends(get_db),
):
    if payload.patient_user_id is None:
        raise HTTPException(status_code=400, detail="Patient is required")
    get_patient_user_or_404(payload.patient_user_id, db)
    assignment = upsert_connection_request(
...
@router.post("/connections/{assignment_id}/accept", response_model=PatientProviderAssignmentResponse)
def accept_connection_request(
    assignment_id: int,
    payload: ConnectionRequestReview,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db),
):
    assignment = get_assignment_or_404(assignment_id, db)
    if assignment.status not in {"pending_provider", "pending_patient"}:
        raise HTTPException(status_code=409, detail="This connection request is no longer pending")
    if assignment.status == "pending_provider" and assignment.provider_user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Only the requested provider can accept this connection")
    if assignment.status == "pending_patient" and assignment.patient_user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Only the requested patient can accept this connection")

    assignment.status = "active"
    assignment.responded_by_user_id = current_user.id
    assignment.responded_at = datetime.now().astimezone()
    assignment.assigned_by_user_id = current_user.id
    assignment.request_message = payload.response_message.strip() if payload.response_message else assignment.request_message
    db.commit()
    return serialize_assignment(get_assignment_or_404(assignment_id, db))


@router.post("/connections/{assignment_id}/reject", response_model=PatientProviderAssignmentResponse)
def reject_connection_request(
    ...
    if assignment.status not in {"pending_provider", "pending_patient"}:
        raise HTTPException(status_code=409, detail="This connection request is no longer pending")
    if assignment.status == "pending_provider" and assignment.provider_user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Only the requested provider can reject this connection")
    if assignment.status == "pending_patient" and assignment.patient_user_id != current_user.id:
        raise HTTPException(status_code=403, detail="Only the requested patient can reject this connection")
