...
from app.models import dose_confirmation, dose_reminder, medication, medication_request, patient_provider_assignment, provider_profile, schedule, user
from app.routes import auth, doses, medication_requests, medications, protected, provider_portal, schedules
from app.services.reminders import start_reminder_worker, stop_reminder_worker
...
def run_startup_migrations():
    ...
    assignment_columns = {column["name"] for column in inspector.get_columns("patient_provider_assignments")}
    ...
    with engine.begin() as connection:
        ...
        if "initiated_by_user_id" not in assignment_columns:
            connection.execute(text("ALTER TABLE patient_provider_assignments ADD COLUMN initiated_by_user_id INTEGER"))
            connection.execute(text("UPDATE patient_provider_assignments SET initiated_by_user_id = assigned_by_user_id WHERE initiated_by_user_id IS NULL"))
        if "request_message" not in assignment_columns:
            connection.execute(text("ALTER TABLE patient_provider_assignments ADD COLUMN request_message VARCHAR(500)"))
        if "responded_by_user_id" not in assignment_columns:
            connection.execute(text("ALTER TABLE patient_provider_assignments ADD COLUMN responded_by_user_id INTEGER"))
        if "responded_at" not in assignment_columns:
            connection.execute(text("ALTER TABLE patient_provider_assignments ADD COLUMN responded_at DATETIME"))
